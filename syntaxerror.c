...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/12 12:43:49 by selhilal          #+#    #+#             */
/*   Updated: 2023/06/17 16:49:08 by selhilal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	secend_syntax(char *str, int i, t_lexer *token)
{
	while (str[i])
	{
		...
		if (str[i] == token->out && str[i + 2] == token->out)
			return (4);
		if (str[i] == token->pipe && str[i + 2] == token->pipe)
			return (5);
		i++;
	}
	return (1);
...
void	parsing_token(char *str, t_lexer *token)
{
	...
	while (str[i])
	{
		if (secend_syntax(str, i, token) == 0 || secend_syntax(str, i, token) == 5)
		{
			printf("syntax error near unexpected token `|'");
			exit(1);
		}
